...
- [ ] T002 Create requirements.txt with dependencies: fastapi, uvicorn, cohere, qdrant-client, psycopg2-binary, python-dotenv, pydantic, pytest
- [ ] T003 Create .env.example file with placeholder values for COHERE_API_KEY, QDRANT_URL, QDRANT_API_KEY, DATABASE_URL
- [ ] T004 Set up configuration module to load environment variables in src/config/
- [ ] T005 Initialize git repository with proper .gitignore for Python project
- [ ] T006 Create main.py application entry point
- [ ] T007 Set up basic logging configuration in src/config/logging.py
- [ ] T008 Create tests/ directory structure with unit, integration, and contract subdirectories

---

## Phase 2: Foundational Components

- [ ] T009 [P] Create BookContent model in src/models/document.py
- [ ] T010 [P] Create ContentChunk model in src/models/document.py
- [ ] T011 [P] Create ChatSession model in src/models/chat_session.py
- [ ] T012 [P] Create UserQuery model in src/models/response.py
- [ ] T013 [P] Create Response model in src/models/response.py
- [ ] T014 [P] Create QueryContext model in src/models/response.py
- [ ] T015 Create database service for PostgreSQL interactions in src/services/database.py
- [ ] T016 Create vector storage service for Qdrant interactions in src/services/retrieval.py
- [ ] T017 Create embedding service using Cohere in src/services/embedding.py
- [ ] T018 Create initial FastAPI application with basic configuration in src/api/main.py
- [ ] T019 Set up dependency injection for database connections in src/api/deps.py
...
- [ ] T020 [P] [US1] Create ingestion service in src/services/ingestion.py
- [ ] T021 [P] [US1] Implement document parsing for PDF format in src/services/ingestion.py
- [ ] T022 [P] [US1] Implement document parsing for ePub format in src/services/ingestion.py
- [ ] T023 [P] [US1] Implement document parsing for text format in src/services/ingestion.py
- [ ] T024 [P] [US1] Create recursive character text splitter for chunking in src/services/ingestion.py
- [ ] T025 [P] [US1] Implement content chunking with 512 token max and 150 token overlap in src/services/ingestion.py
- [ ] T026 [P] [US1] Create BookContent CRUD operations in src/services/database.py
- [ ] T027 [P] [US1] Create ContentChunk CRUD operations in src/services/database.py
- [ ] T028 [US1] Implement embedding generation using Cohere embed-v3 in src/services/embedding.py
- [ ] T029 [P] [US1] Store content chunks in Qdrant with vector representations in src/services/retrieval.py
- [ ] T030 [P] [US1] Create generation service using Cohere command-r-plus in src/services/generation.py
- [ ] T031 [P] [US1] Implement RAG pipeline: query embedding → vector search → context augmentation → response generation in src/services/generation.py
- [ ] T032 [P] [US1] Create query service with relevance threshold filtering (>0.7) in src/services/generation.py
- [ ] T033 [P] [US1] Implement citation creation with page numbers and section titles in src/services/generation.py
- [ ] T034 [P] [US1] Create ChatSession CRUD operations in src/services/database.py
- [ ] T035 [P] [US1] Create UserQuery and Response CRUD operations in src/services/database.py
- [ ] T036 [P] [US1] Create ingest endpoint POST /ingest in src/api/routes/ingest.py
- [ ] T037 [P] [US1] Create query endpoint POST /query in src/api/routes/query.py
- [ ] T038 [P] [US1] Implement validation for query requests in src/api/routes/query.py
- [ ] T039 [P] [US1] Implement rate limiting for API endpoints in src/api/main.py
- [ ] T040 [US1] Add response validation to prevent hallucinations and ensure source attribution
- [ ] T041 [US1] Implement session management with unique tokens for conversation history
- [ ] T042 [US1] Add performance monitoring and response time tracking under 2 seconds
- [ ] T043 [US1] Write unit tests for ingestion service
- [ ] T044 [US1] Write unit tests for embedding service
- [ ] T045 [US1] Write unit tests for retrieval service
- [ ] T046 [US1] Write integration tests for RAG pipeline
- [ ] T047 [US1] Write API contract tests for /ingest and /query endpoints
...
- [ ] T067 Create query-selected endpoint POST /query-selected in src/api/routes/query.py
- [ ] T068 [P] [US2] Implement selected text handling in generation service
- [ ] T069 [US2] Add selected-text specific validation to new endpoint
...
- [ ] T079 Implement comprehensive error handling and logging
- [ ] T080 Add input validation and sanitization to prevent injection attacks
- [ ] T081 Implement proper authentication and user privacy features
- [ ] T082 Add comprehensive API documentation at /docs and /redoc
- [ ] T083 Implement caching for frequently accessed embeddings
- [ ] T084 Add health check endpoint GET /health
- [ ] T085 Create deployment configuration files (Dockerfile, docker-compose.yml)
- [ ] T086 Implement monitoring and alerting setup
- [ ] T087 Write comprehensive README.md with setup and usage instructions
